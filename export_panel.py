...
def export_panel_latest(connection_string: str, output_path: str = "outputs/panel_latest.csv"):
    ...
    WITH all_keys AS (
        ...
        UNION
        SELECT firm_id, fiscal_year, snapshot_id FROM fact_market_year
        UNION
        SELECT firm_id, fiscal_year, snapshot_id FROM fact_ownership_year
        UNION
        SELECT firm_id, fiscal_year, snapshot_id FROM fact_innovation_year
        UNION
        SELECT firm_id, fiscal_year, snapshot_id FROM fact_firm_year_meta
    ),
